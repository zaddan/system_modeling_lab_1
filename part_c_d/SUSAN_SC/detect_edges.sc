...
behavior detectedges(i_receiver in_port,i_sender out_portR, i_sender out_portMid)
{
    ...
    setup_brightness_lut_fsm setbrightness(local_port); //outport - local_port
    susan_edges_fsm sedge(in_port,local_port,out_portR, out_portMid);//find how are they connected
    void main(void)
    {
      par { 
            setbrightness;
